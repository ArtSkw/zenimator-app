...
5. **Declare content parameters.** When a scene carries user-facing content —
   a label, an accent fill, a **gradient ramp**, a size — declare it in
   `controls.json`'s `parameters` array WITH ITS KIND, and publish every `sid`
   in `slots`. Shape inference cannot see a gradient, so an undeclared ramp is
   simply not editable and a flat swatch beside it silently takes the credit.
   Then run `node scripts/check-parameters.mjs <project> scene-1` — it must
   exit 0. It also NOTES any gradient with no parameter; if the brief names
   that subject, it needs one. Rules: player-contract "Content Parameters".

6. **Document learnings**: after a non-trivial scene, append what you learned
...
7. When done, print a single final line:

#!/usr/bin/env node
/**
 * Generates a seamlessly-looping Lottie JSON for "loop-zenek-our-nzfu.svg"
 * — Zenek perched on his badge, gears turning, pen jotting notes.
 * (Same illustration + brief as the dataprocessing scene; the source SVG's
 *  path data is byte-identical, so this reuses that proven, documented rig
 *  verbatim — see docs/dataprocessing-loop-animation.md.)
 * Output: public/projects/loop-zenek-our-nzfu/scene-1/lottie.json
 *
...
const OUT_DIR = join(__dirname, '../public/projects/loop-zenek-our-nzfu/scene-1')
...
// ---- pencil (float + tap-down every other cycle) ----
{
  ...
  for (let c = 0; c < FLOAT_CYCLES; c++) {
    ...
    if (isTap) {
      ...
      points.push({ t: base + 112, v: [-1.5, 2, 0], ease: 'entranceSharp' })
      points.push({ t: base + 120, v: [1, 2, 0], ease: 'settleSoft' })
    }
  }
...
const doc = {
  v: '5.9.0', fr: FPS, ip: 0, op: T, w: W, h: H, nm: 'Loop Zenek — Data Processing',
  ddd: 0,

 ...
 *
 * Output: public/projects/paymentconfirmation-story-three-zezm/scene-1/lottie.json
 */
...
const SLUG = 'paymentconfirmation-story-three-zezm'
...
const DRAW_START = ANTIC_END      // same clock as the ribbon's trim-draw
const DRAW_END = RISE_END
...
// THE GLEAM — a travelling trim WINDOW on the ribbon's own stroke. The
// original stacked-band Merge-Paths clip rendered in Skottie but NOT in the
// HTML export (lottie-web has no mm translation — the raw bands painted from
// frame 0), and check-motion now fails mm outright. Three white strokes on
// the ribbon's own path, decreasing width, low alpha, one travelling trim
// window: inside the ribbon by construction, soft-ended by its round caps,
// export-safe everywhere. See player-contract "Export Compatibility".
const GLEAM_IN = T + Math.round(LOOP * 0.10)
const GLEAM_OUT = T + Math.round(LOOP * 0.80)
const GLEAM_WINDOW = 18
const GLEAM_LAYERS = [
  { w: RIBBON_W * 0.98, a: 10 },
  { w: RIBBON_W * 0.72, a: 13 },
  { w: RIBBON_W * 0.44, a: 16 },
]
function gleamTrim() {
  const s = anim([
...
const ribbonLyr = shapeLayer('mascot-ribbon', ribbonInd, null, [
  group([pathShape(ribbonSeg), gradientStroke(RIBBON_W), trimShape(0, ribbonDrawTrim)])
])
...
const controls = {
  ...
  motionExceptions: [
    { layer: 'eyes', reason: "designer direction: only the body and its white face morph — the eyes keep their drawn shape, so their scale track is the exact inverse of the body's squash/stretch (product 100%), never a blink or a squint." },

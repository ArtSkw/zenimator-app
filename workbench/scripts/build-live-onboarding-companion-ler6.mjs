#!/usr/bin/env node
/**
 * Generates the INTRO+LOOP Lottie for "live-onboarding-companion-ler6.svg" —
 ...
 * scripts/build-live-onboarding-companion-eh0n.mjs; see
 * docs/live-onboarding-companion-eh0n-animation.md for the full rig writeup.
 * Output: public/projects/live-onboarding-companion-ler6/scene-1/lottie.json
 *
 ...
 * See docs/live-onboarding-companion-ler6-animation.md for the mood retune
...
const OUT_DIR = join(__dirname, '../public/projects/live-onboarding-companion-ler6/scene-1')
...
function staticShapeLayer(nm, d, paintItems, parent, p) {
  ...
  pushLayer({ nm, shapes, ks: baseTransform(p ? { p } : {}), parent })
}
...
// Stage-safety margin (motion-taste.md "Render-Aware Motion"): ~3% of the
// 240px min dimension.
const MARGIN = 240 * 0.03 // 7.2

// ============================================================
// STAGE SHIFT — recipe-companion-bubble.md's "growable plate must grow away
// from its tail" fix needs headroom ABOVE the bubble at its tallest (2-line,
// see bubble.anchor below) that the ORIGINAL layout didn't have: at
// PLATE_BOTTOM=62 (unchanged, still source-derived), a 58px-tall plate's top
// would land at 62-58=4, short of the margin by 3px+.
//
// A first pass moved the whole composition via ONE shared parent null
// (translate-only). That's correct Lottie and renders identically, but the
// app's own layout/measurement code reads each layer's OWN authored `p`/`a`
// directly — it does not walk the parent chain — so a shift living only on
// an ancestor's transform was invisible to it (measured: still 62/4, as if
// unshifted). Fix: bake SHIFT_Y directly into every top-level layer's OWN
// `p`, leaving `a` untouched wherever a layer already pivots via `a==p`
// (head-rig, chair-seat) — screenPoint = local - a + p, so adding SHIFT_Y
// to `p` alone while `a` stays at the true pivot shifts the whole rig
// uniformly without disturbing what it rotates/scales around. This is the
// SAME rigid-group-translation result (every authored gap preserved
// exactly), just expressed on each layer's own numbers instead of a shared
// ancestor, so it reads correctly regardless of how a consumer measures it.
// The chair legs' own lowest point is ~226 in the un-shifted 240 comp, so
// there's unused room below to spend; shifting down trades a few px of that
// slack for the top headroom the bubble's growth needs.
const SHIFT_Y = 4
...
// BOTH stage edges by MARGIN (7.2px) at bubble.size's autoFit `max`, not
// just the default. Stage is 240x240; PLATE_CX=120 is the true stage-center
// so max[0]'s clearance is symmetric on both edges.
//
// Vertical position is source-derived, not stage-safety-derived: PLATE_TOP/
// PLATE_BOTTOM below restore the SVG's authored plate-to-trail gap (see
// TRAIL below) and are UNCHANGED by the stage-shift fix (they're the
// pre-shift design values — SHIFT_Y is added where `bubble-anchor`'s own
// `p` is actually constructed, below, so every gap computed from these
// numbers stays exactly as authored; only the FINAL on-screen Y adds
// SHIFT_Y). Since the plate now grows upward from a pinned bottom
// (bubble.anchor, below) instead of symmetrically from its center, the
// relevant margin check is at the TALLEST plate (max[1], a 2-line wrap),
// not the old 3-line/73px figure a since-superseded pass checked:
// top-at-max = PLATE_BOTTOM + SHIFT_Y - max[1] = 62 + 4 - 58 = 8, clearing
// the 7px margin by 1px. See the AUTOFIT_MAX derivation below for max[1]'s
// own math.
// ============================================================
...
const PAD_X = 16, PAD_Y = 8 // ZEN tooltip padding spec
// recipe-companion-bubble.md "Wrapped lines need leading": px added to
// LINE_HEIGHT per line ONLY when the string wraps — a single line stays
// exactly 15/19, pixel-true to the source; stacked lines at the bare 19
// line-height read cramped.
const LEADING = 2
...
// recipe-companion-bubble.md "A growable plate must grow AWAY from its
// tail": a rounded rect is centred on its own local origin, so slot-driven
// height growth on `bubble.size` pushes the plate's BOTTOM edge down —
// straight into the trail below (measured: at h=35 the bottom sits at the
// authored 9px gap above trail-large; at h=54 it had crossed 0.5px INTO the
// circle). Fix: the anchor itself is a slot, `bubble.anchor`, default
// `[0, PLATE_DEFAULT_H/2, 0]` — tools rewrite its y to `height/2` on every
// resize. With `a.y = h/2` and `p.y` fixed at PLATE_BOTTOM, the transform
// (`screenY = local - a.y + p.y`) puts the plate's local bottom edge
// (`local = +h/2`) at `h/2 - h/2 + PLATE_BOTTOM = PLATE_BOTTOM` for EVERY
// h — the bottom never moves, and all growth goes upward instead.
...
// companion-ler6.svg): plate rect x=14 y=14 w=176 h=35 (bottom edge y=49);
...
//
// cx/cy arguments below are the pre-shift design values (unchanged from the
// source SVG); SHIFT_Y is added where each circle's own position track is
// built, so every gap computed from cx/cy stays exact regardless of
// SHIFT_Y — see BUBBLE section above for why the shift is baked per-layer
// rather than via a shared ancestor.
// ============================================================
...
function trailCircle(nm, cx, cy, r, points, floatPhase = 0) {
  ...
  const posPoints = []
  for (let t = 0; t <= OP; t += 2) posPoints.push({ t, v: [cx, cy + SHIFT_Y - FLOAT_AMOUNT * floatEnvelope(t + floatPhase), 0], ease: 'linear' })
  if (posPoints[posPoints.length - 1].t !== OP) posPoints.push({ t: OP, v: [cx, cy + SHIFT_Y - FLOAT_AMOUNT * floatEnvelope(OP + floatPhase), 0] })
...
// ============================================================
staticShapeLayer('chair-leg-back-a', SVG_PATHS.legBackA, [strokeItem('#222222', 2.0532)], undefined, [0, SHIFT_Y, 0])
staticShapeLayer('chair-leg-back-b', SVG_PATHS.legBackB, [strokeItem('#222222', 2.0532)], undefined, [0, SHIFT_Y, 0])
staticShapeLayer('chair-leg-back-c', SVG_PATHS.legBackC, [strokeItem('#222222', 2.0532)], undefined, [0, SHIFT_Y, 0])
staticShapeLayer('chair-leg-front', SVG_PATHS.legFront, [strokeItem('#222222', 2.0532)], undefined, [0, SHIFT_Y, 0])
...
const headRigInd = pushLayer({
  ...
  ty: 3,
  ks: {
    // a stays at the TRUE (pre-shift) pivot — rotation/scale keep pivoting
    // exactly where they always did; p adds SHIFT_Y on top (screenPoint =
    // local - a + p), so the whole rig translates uniformly without
    // disturbing what it rotates around. Not a==p anymore (that was "no net
    // translation"); this is "net translation of exactly SHIFT_Y".
    a: { a: 0, k: [headPivot[0], headPivot[1], 0] },
    p: { a: 0, k: [headPivot[0], headPivot[1] + SHIFT_Y, 0] },
...
// ============================================================
{
  ...
  // a stays at the true pivot (unshifted); p adds SHIFT_Y — same "shift via
  // p only" idiom as head-rig above.
  const ks = baseTransform({ a: [seatPivot[0], seatPivot[1], 0], p: [seatPivot[0], seatPivot[1] + SHIFT_Y, 0] })
...
// drink-rig has no rig-pivot pattern to preserve (a is [0,0,0], p carries the
// small sip-lift delta directly) — SHIFT_Y is added to every sampled point's
// Y component directly, same net effect as the "shift via p" idiom above.
const drinkPosPoints = []
for (let t = 0; t <= OP; t += HEAD_SAMPLE_STEP) { const v = sipEnvelope(t); drinkPosPoints.push({ t, v: [v[0], v[1] + SHIFT_Y, 0], ease: 'linear' }) }
if (drinkPosPoints[drinkPosPoints.length - 1].t !== OP) { const v = sipEnvelope(OP); drinkPosPoints.push({ t: OP, v: [v[0], v[1] + SHIFT_Y, 0] }) }
...
const doc = {
  ...
  slots: {
    ...
    // recipe-companion-bubble.md "A growable plate must grow AWAY from its
    // tail": default matches bubble-anchor's own current anchor, so the
    // file renders identically to before unless a tool overrides it.
...
// max is REQUIRED (recipe-companion-bubble.md #3), derived from the real
// geometry, not guessed:
// - max[0] (width): the widest plate that clears BOTH stage edges by
//   MARGIN, given PLATE_CX=120 is the exact stage-center — 240 - 2*MARGIN.
//   Unaffected by the vertical stage-shift below.
// - max[1] (height): the tallest WHOLE-LINE plate the layout has headroom
//   for now that growth goes upward (bubble.anchor, above) instead of
//   symmetric-from-center. Plate bottom (post-shift) is PLATE_BOTTOM +
//   SHIFT_Y = 62+4 = 66; max[1] = (plate bottom - margin), rounded DOWN to a
//   whole number of lines at lh=LINE_HEIGHT+LEADING=21 with 2*PAD_Y=16
//   padding: a 2-line wrap is 2*21+16=58, top lands at 66-58=8, clearing the
//   7px margin by 1px. A 3-line wrap (3*21+16=79) would need top=66-79=-13 —
//   deeply negative, no reasonable stage-shift recovers that — so 2 lines is
//   the real ceiling this layout supports; a longer string widens toward
//   max[0] instead (wraps at 2 lines max).
const AUTOFIT_MAX_W = 240 - 2 * MARGIN // 225.6
const AUTOFIT_MAX_H = 2 * (LINE_HEIGHT + LEADING) + 2 * PAD_Y // 58
const AUTOFIT_MAX = [AUTOFIT_MAX_W, AUTOFIT_MAX_H]

const controls = {
  controls: [
    { sid: 'bubble.text', label: 'Bubble text' },
    { sid: 'bubble.size', label: 'Bubble size', autoFit: { text: 'bubble.text', padding: [PAD_X, PAD_Y], min: [90, 35], max: AUTOFIT_MAX, leading: LEADING } },
    ...
    // recipe-companion-bubble.md "A growable plate must grow AWAY from its
    // tail": tools rewrite this slot's y to height/2 on every bubble.size
    // resize, pinning the plate's bottom edge so growth goes upward.

 ...
 *
 * THIS RUN (slug 3soj): the three source SVGs are byte-identical (md5-diffed
 * before authoring) to the zezm/h3oo/6v93/4obq/dzbr/wyga/4pik field tests.
 * This brief's own language matches zezm's round-3 (final) brief and
 * 4obq's/dzbr's/wyga's/4pik's own briefs on every point those scenes already
 * settled: ticks are pure decals with no pass-accent; air-flow marks show
 * ONLY on the descent (never the rise, on both the loop bounces and the
 * beat-3 entrance); eyes keep their exact drawn shape via counter-scale; and
 * the entrance/gesture-overlap wording ("Anchor this on when he crosses into
 * view, not on when his anticipation starts... He should clear the bottom
 * edge at roughly the point where the swoosh is two-thirds drawn... Overlap
 * them, never queue them") is 4pik's own visibility-anchored fix, verbatim in
 * substance. Base script: 4pik — the most current, field-tested descendant
 * (4obq: ONE-PEN sequential green-line draw; wyga Edit 1/2: green line timed
 * independent of the mascot, entrance overlapping rather than queuing; 4pik
 * Edit 1: entrance anchored to VISIBILITY of the pen-lift, not the pen-lift
 * frame itself — exactly what this brief's wording asks for).
 *
 * PORTING IS NOT AUTHORING — two things in THIS brief are NOT in 4pik (or
 * any prior script in this lineage), found by reading the brief itself
 * rather than trusting the file/geometry match:
 *   1. "When the check's pen lifts, the finished mark STAMPS: a small
 *      settle-pop of two to three percent on its own pivot... Then let it
 *      hold a beat." No prior script pops the check-mark itself — it just
 *      holds flat after its trim completes. NEW: `checkStampAnim()`.
 *   2. "The bright swoosh and its spark answer his landings too: on each
 *      impact they dip a couple of pixels and recover, a frame or two behind
 *      him, at low amplitude derived from his own bounce... The pale ribbon
 *      stays still." Every prior script in this lineage declares swoosh AND
 *      spark AND ribbon as fully stage-fixed (`motionExceptions`, zero
 *      motion of their own) — that was a correct reading of those briefs'
 *      weaker "everything rests exactly where artwork 3 draws it" / "the
 *      swoosh and spark land with him" language. THIS brief is explicit and
 *      unambiguous that swoosh+spark get a small landing-synced dip while
 *      the ribbon alone stays fully static. NEW: `landingDipPosition()`.
 * Every other constant below was re-checked against this brief's own text
 * and the CURRENT skill references before shipping (player-contract.md's
 * opacity-does-not-cascade/precomp note + Export Compatibility section,
 * motion-taste's Aliveness Contract gates 1-19 — re-read this session, still
 * 1-19 — chapterization's Grounded Handoffs checklist), not carried on the
 * strength of the prior scripts' git history. This scene has NO speech
 * bubble/tooltip/text layer, so recipe-companion-bubble.md's HARD CONTRACT
 * items about a bubble text layer, autoFit, and a `.textPos` slot are
 * inapplicable by construction (declared here, not silently skipped); only
 * its "Intro + Loop" marker/idle-alive-from-frame-0 mechanism applies, and
 * it does.
 *
 * Output: public/projects/paymentconfirmation-story-three-3soj/scene-1/lottie.json
 */
...
const SLUG = 'paymentconfirmation-story-three-3soj'
...
const BEAT2 = 104  // 1.73s — confirmation: the exit cascade clears the face
                   // BEFORE the check draws, the check's own stamp-settle
                   // (164->172, NEW this brief), then a 4f pure hold to POP_START
...
const CHECK_END = CHECK_START + CHECK_DRAW  // 164
// NEW this brief: "When the check's pen lifts, the finished mark STAMPS: a
// small settle-pop of two to three percent on its own pivot, over a few
// frames... Then let it hold a beat." No prior scene in this lineage pops
// the check-mark itself. The stamp fits entirely inside the exit cascade's
// existing 12f hold to POP_START (164 -> 176), leaving BEAT2's own duration
// (and everything computed from POP_START onward) untouched.
const STAMP_PEAK = CHECK_END + 4            // 168
const STAMP_END = CHECK_END + 8             // 172 — then a 4f pure hold to POP_START (176)

// Beat 3: badge pop, then the green line (ribbon then swoosh), overlapping
// straight into the mascot's launch — ONE continuous phrase, not a queue.
// The green line's own pace is unchanged (a 50f swoosh at the artwork's own
// zigzag speed, feeding a length-proportional ribbon ahead of it) and is
// computed FIRST here, independent of the mascot.
//
// This brief's own wording ("Anchor this on when he crosses into view, not
// on when his anticipation starts... he should clear the bottom edge at
// roughly the point where the swoosh is two-thirds drawn... Overlap them,
// never queue them: there must be no frame where the stroke has finished and
// nothing else has yet moved") is 4pik's own field-tested fix, in substance:
// anchor the entrance to VISIBILITY of the pen-lift, not to the pen-lift
// frame itself (an easing curve's near-zero start slope can keep a rig
// invisible for ~30+ frames after its launch keyframe even when that
// keyframe IS the pen-lift frame — measured on 4pik's own first cut).
...
const RIBBON_DRAW_START = POP_START + 8     // 184 — over the badge's last fade
                                            // frames, a deliberate cross-dissolve
const SWOOSH_DRAW_DUR = 50                  // unchanged pace — the artwork's own zigzag speed
const SWEEP_VS_SCRIBBLE = 1.4               // a long smooth sweep runs hotter than a tight scribble
const RIBBON_DRAW_DUR = Math.round(
  SWOOSH_DRAW_DUR * (pathLength(P3.ribbon[0]) / pathLength(P3.swoosh[0])) / SWEEP_VS_SCRIBBLE)
const RIBBON_DRAW_END = RIBBON_DRAW_START + RIBBON_DRAW_DUR   // no gap: the pen never lifts
const DRAW_START = RIBBON_DRAW_END          // swoosh picks the line up here
const DRAW_END = DRAW_START + SWOOSH_DRAW_DUR

const LAST_THIRD = Math.round(SWOOSH_DRAW_DUR / 3)  // 17f
// VISIBILITY_LEAD is tuned, not assumed: the naive position-only crossing
// test (used in this lineage's prior docs) ignores that mascotScale() is
// still deep in its ANTIC_SQUASH compression through the early rise, which
// shrinks the body's effective height above the pivot and delays the ACTUAL
// rendered first-visible-pixel frame well past the position track's own
// crossing. Measured this session by rendering the actual output at zoom 6,
// frame by frame: VISIBILITY_LEAD=20 (ANTIC_END=236) put the first visible
// sliver at 241 — ~1.7f AFTER the swoosh's own two-thirds mark (239.3),
// which is "too late" per the brief's own verification wording. Increased to
// 23 so the sliver lands AT/just before the two-thirds mark instead (re-
// verified below after rebuilding).
const VISIBILITY_LEAD = 23
const ANTIC_END = DRAW_END - VISIBILITY_LEAD  // rise takes over 20f before the pen runs out — anchored to visibility, not the pen-lift
const ANTIC_START = ANTIC_END - LAST_THIRD  // anticipation begins here, still offscreen
const RISE_END = ANTIC_END + 50             // overshoot peak, unchanged pace
const LAND_END = RISE_END + 22              // settle, unchanged pace — lands well after the spark
const T = LAND_END + 14              // intro/loop boundary — 14f hold after settle,
                                     // per player-contract ("a few frames after it settles")
...
console.log(`two-thirds swoosh mark = ${(DRAW_START + (2 / 3) * SWOOSH_DRAW_DUR).toFixed(1)}, ANTIC_END(rise takeover)=${ANTIC_END}`)
...
// sweeps (the brief cuts the per-tick pass accent explicitly — "they do not
// swell, brighten, pulse, or react to the hand in any way"), and retract as
// one assembly once the check takes over.
...
function hubAbsorbExitAnim() {
  // Swallow the hand, then leave — fast. Fully gone at HUB_EXIT_END (132),
  // well before the check finishes and well before the check's line crosses
  // the middle of the face, so the two never read at once.
  const kfs = [
    { t: 0, s: [100, 100, 100], ease: EASE.linear },
...
// NEW this brief: the STAMP. "When the check's pen lifts, the finished mark
...
// beat." Pivots on the check's OWN artwork center (scale-pivot gate) — the
// same pattern the hub/ticks/sparkles already use, applied here for the
// first time in this lineage.
const checkBox = bbox([CHECK_SEG])
const STAMP_PEAK_PCT = 2.5   // "two to three percent"
function checkStampAnim() {
  const kfs = [
    { t: 0, s: [100, 100, 100], ease: EASE.linear },
    { t: CHECK_END, s: [100, 100, 100], ease: EASE.expressivePop },
    { t: STAMP_PEAK, s: [100 + STAMP_PEAK_PCT, 100 + STAMP_PEAK_PCT, 100], ease: EASE.settleSoft },
    { t: STAMP_END, s: [100, 100, 100] },
  ]
  return anim(kfs)
}
const checkInd = IND++
...
], { p: sk([checkBox.cx, checkBox.cy, 0]), a: sk([checkBox.cx, checkBox.cy, 0]), s: checkStampAnim() })
...
function mascotPosition() {
  ...
  const kfs = [
    ...
    // settleSoft (near-vertical start slope), not exitAccelerate (near-zero
    // start slope) — the launch must be visible almost immediately after
    // ANTIC_END, not tens of frames later once the easing finally picks up.
    // A fast liftoff decelerating into the overshoot also reads as the more
    // physical jump.
    { t: ANTIC_END, s: anticOff, ease: EASE.settleSoft },
...
// The eyes KEEP their drawn shape (brief: "they never squash, stretch, or
// squint... their own scale has to cancel the body's out"). They are
// children of mascot-root, so they would inherit its squash/stretch — this
// inverts the parent's scale at every one of its own keyframes, so the
// product is 100% throughout and the arcs stay exactly as artwork 3 draws
// them. They still ride the body's position and lean, which is what keeps
// them glued to the face.
...
// Air-flow marks (the two thin curves above the head): "the trace of the air
// he has just moved through" — they exist only WHILE he falls: invisible at
// rest, at every landing, and through the whole rise/peak, visible only
// through the descent (and the entrance's own fall from its overshoot), with
// a small lagging swish rotation so they read as displaced air. The
// visibility cycle lives on the SHAPE layer, never the rig null — a parent's
// `o` does not cascade in this player (see the badge pop note).
const AIR_LAG = 5
...
function airflowOpacity() {
  // Cartoon speed lines TRAIL the move that just happened, they don't lead
  // it: nothing at the apex, streaming in as the descent gets going, gone at
  // the impact. Zero at every PEAK and every LANDING — and since LANDINGS
  // includes both T and OP, the loop boundary matches by construction.
...
// The greens are one continuous drawn line — the ribbon's head (198,112)
// sits on the swoosh's own start (173,112), and the swoosh's exit (222,148)
// hands into the spark. Everything settles well before T; T is keyed
// explicitly at the same rest value OP is forced to (player-contract.md,
// "a boundary value must come from evaluating the SAME function at that
// boundary").
// ONE PEN, TWO HALVES, IN SEQUENCE. A hand draws a connected line along its
// own length: the ribbon first (left to right), then the swoosh from where
// the pen arrives. Durations are length-proportional so the pen does not
// change gear at the junction; the sweep runs slightly hotter than the
// scribble because a long smooth arc genuinely does. Both halves ease as an
// S-curve, so the small slowdown at the handover reads as the hand rounding
// a corner. Timing is computed in the TIMELINE section above, ahead of the
// mascot's own entrance — the green line draws on an empty stage first, and
// his launch is timed off it.
// The swoosh is DRAWN, not wiped ("like you would draw the letter Z with
// your hand"). A flattened polygon can't take a trim path, so the tube
// itself is keyframed: `swooshTube.at(p)` returns the tube truncated at
// pen-progress p with a constant vertex count (Skottie's shape interpolation
// contract), and its round end-cap IS the pen tip.
const DRAW_STEPS = 20
...
// NEW this brief: "The bright swoosh and its spark answer his landings too:
// on each impact they dip a couple of pixels and recover, a frame or two
// behind him, at low amplitude derived from his own bounce — they share his
// ground, but they never move on their own clock, and between impacts they
// rest exactly where artwork 3 draws them. The pale ribbon stays still."
// Every prior scene in this lineage declared swoosh+spark+ribbon fully
// stage-fixed; THIS brief is explicit that swoosh and spark alone get a
// small landing-synced dip. Amplitude is DERIVED from JUMP_H (not an
// independent number), and the dip is keyed directly off the shared
// LANDINGS array with a small phase lag — not a separate clock — so it
// reads as an answer to his weight, the same technique the shadow already
// uses. Explicit rest keys AT every landing (including T and OP) keep the
// loop seam exact regardless of what happens between them; the dip+recover
// pair is only emitted for landings strictly before OP because nothing
// renders past `op` — the wrap to T supplies the next impact.
const SWOOSH_LAND_LAG = 2        // "a frame or two behind him"
const SWOOSH_LAND_DIP = JUMP_H * 0.125   // 2.5px — "a couple of pixels," derived from his own bounce amplitude
function landingDipPosition(baseX, baseY) {
  const kfs = [{ t: 0, s: [baseX, baseY, 0], ease: EASE.settleSoft }]
  for (let i = 0; i < LANDINGS.length; i++) {
    const L = LANDINGS[i]
    kfs.push({ t: L, s: [baseX, baseY, 0], ease: EASE.settleSoft })
    if (L === OP) continue
    kfs.push({ t: L + SWOOSH_LAND_LAG, s: [baseX, baseY + SWOOSH_LAND_DIP, 0], ease: EASE.settleSoft })
    kfs.push({ t: L + SWOOSH_LAND_LAG + 8, s: [baseX, baseY, 0], ease: EASE.settleSoft })
  }
  return anim(sortedByT(kfs), EASE.settleSoft)
}

function sparkEntrance() {
  // The pen lifts as the stroke finishes — the spark pops right at that
  // handoff, the same instant the mascot's rise takes over (ANTIC_END ==
  // DRAW_END - VISIBILITY_LEAD), so something is already resolving on stage
  // the moment the line completes. He lands after the spark has settled, not
  // with it.
  const c = DRAW_END - 2
  const s = anim([
...
// reflection" gleam travels along it once per loop. THE PALE RIBBON STAYS
// STILL throughout (brief) — no position track at all, unlike swoosh/spark.
...
// THE GLEAM — a travelling trim WINDOW on the ribbon's own stroke. Three
// white strokes on the ribbon's OWN path, of decreasing width and low alpha,
// sharing one trim window that travels the length of the path — inside the
// ribbon by construction, no Merge Paths (which do not survive the HTML
// export — player-contract.md's Export Compatibility section).
...
], { p: landingDipPosition(swooshBox.cx, swooshBox.cy), a: sk([swooshBox.cx, swooshBox.cy, 0]), s: ambientBreathScale(), o: swooshDrawOpacity() })
...
], { p: landingDipPosition(sparkBox.cx, sparkBox.cy), a: sk([sparkBox.cx, sparkBox.cy, 0]), s: sparkAnims.s, o: sparkAnims.o })
...
const controls = {
  ...
  motionExceptions: [
    ...
    { a: 'ribbon', b: 'outline', reason: "brief: \"The pale ribbon stays still\" — it holds its source position for the whole loop while the body bounces near it; not welded to the mascot." },
    { a: 'ribbon', b: 'belly', reason: "same as above." },
    { a: 'swoosh', b: 'outline', reason: "brief: \"The bright swoosh and its spark answer his landings too: on each impact they dip a couple of pixels and recover, a frame or two behind him, at low amplitude derived from his own bounce\" — a small world-response DERIVED from the mascot's own LANDINGS clock (same technique as the shadow), not a parented/welded part of the body." },
    { a: 'swoosh', b: 'belly', reason: "same as above." },
    { a: 'spark', b: 'outline', reason: "same as above — the spark shares the swoosh's exact landing-dip clock and amplitude." },
    ...
    { a: 'ribbon', b: 'swoosh', reason: "brief: \"The pale ribbon stays still\" while the swoosh now dips a couple of px on every landing — their touching edge (the ribbon's head meeting the swoosh's start) genuinely opens and closes by the dip amplitude on each impact, a brief-directed divergence between a static element and a landing-responsive one." },
    { a: 'ribbon', b: 'eyes', reason: "stage-fixed ribbon passing near the face, not welded to it." },
    { a: 'swoosh', b: 'airflow', reason: "the swoosh's small landing-dip passes near the air-flow marks; both are independently driven off the mascot's own LANDINGS clock (derived, not welded)." },
    { a: 'swoosh', b: 'eyes', reason: "the swoosh's small landing-dip passes near the face, not welded to it." },
    { a: 'swoosh', b: 'spark', reason: "swoosh and spark share the IDENTICAL landing-dip function (same lag, same amplitude) so they move in lockstep with each other; the residual is the spark's own pop scaling around its own artwork centre (scale-pivot gate) at the entrance, not an independent clock." },
  ],
}

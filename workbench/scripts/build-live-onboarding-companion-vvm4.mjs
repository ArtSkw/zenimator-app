#!/usr/bin/env node
/**
 * Generates the INTRO+LOOP Lottie for "live-onboarding-companion-vvm4.svg" —
 ...
 *
 * `assets/live-onboarding-companion-vvm4.svg` is byte-identical to
 * `live-onboarding-companion-i8ek.svg`, and this brief is the same
 * bodybuilder mascot scene ("the pump", "Making great progress" bubble). This
 * is a wholesale port (`cp` + slug swap) of the i8ek build script, which is
 * itself the motion-craft-pass revision of the rig (elbow hinge, bicep
 * bulge, fist lag) — see that revision's rationale inline below, unchanged.
 *
 ...
 *    peak ("fists tighten fractionally"), now riding the same lag as the
 *    wrist so the tighten arrives with the fist's delayed settle.
 *  - a brief 1.4px isometric QUIVER (a windowed wobble, retimed this pass
 *    to a 5-frame/6Hz half-cycle over a 30-frame window — was a 2.25-frame/
 *    ~15Hz buzz, per motion-taste "Accents must be readable") layered on
 *    top of the arm position, active during the squeeze's hold plateau —
 *    the loop's snappy accent.
 ...
 *
 * Output: public/projects/live-onboarding-companion-vvm4/scene-1/lottie.json
 */
...
const OUT_DIR = join(__dirname, '../public/projects/live-onboarding-companion-vvm4/scene-1')
...
const SVG_SRC = readFileSync(join(__dirname, '../assets/live-onboarding-companion-vvm4.svg'), 'utf8')
function tagById(id) {
  const needle = `id="${id}"`
...
// Isometric quiver — a windowed wobble active during (and briefly bracketing)
// the hold plateau [60,78] of each cycle, the loop's one snappy accent.
// motion-taste "Accents must be readable": the ORIGINAL window (18 frames,
// 4 oscillation cycles) gave a 2.25-frame half-cycle — ~15Hz at 60fps, too
// fast to resolve, reads as a buzz. Widened to a 30-frame window (>= the
// ~0.4s/24f floor) with 3 cycles: half-cycle = 30 / (2*3) = 5 frames = 6Hz
// at 60fps, inside the "readable, felt" 5-6f/<=8Hz band. Amplitude
// (QUIVER_AMOUNT) is unchanged — frequency alone was the defect.
const QUIVER_LO = 54, QUIVER_HI = 84 // local-cycle frames (t % IDLE) — the hold [60,78] plus 6f pre/post, NOT the absolute-time T=54 loop marker
...
function buildArm(side, knuckleSrc) {
  ...
  // bbox center.
  {
    const fistScalePoints = denseSample((t) => 100 + FIST_GAIN_PCT * squeezeEnvelope(t - FIST_LAG_FRAMES))
...
// squeeze=0 (true rest, including frame 0) must reproduce the UNMODIFIED
// source silhouette exactly (scale=1) — the arm-rig already follows this
// convention (dx=0 at squeeze=0). Chest recedes slightly AS the arms flex
// harder (squeeze -> 1), the "counter-phase" reading.
function chestScaleAt(t) { return 1 - CHEST_PUFF_PCT * squeezeEnvelope(t) }
...
// squeeze=0 (true rest) must reproduce the unmodified source shadow exactly
// (scale=100) — same "rest == source" convention as the arm-rig and chest.
// The shadow narrows slightly as the arms pull into their deepest flex
// (body mass gathered up/tight) and widens back to its full baseline
// footprint as the squeeze releases — "widens slightly as the body settles."
const SHADOW_WIDEN_X_PCT = 5, SHADOW_WIDEN_Y_PCT = 1.5
const shadowScalePoints = denseSample((t) => {
  const squeeze = squeezeEnvelope(t)
  return [100 - SHADOW_WIDEN_X_PCT * squeeze, 100 - SHADOW_WIDEN_Y_PCT * squeeze, 100]
})

#!/usr/bin/env node
/**
 * Generates the INTRO+LOOP Lottie for "live-onboarding-companion-phq0.svg" —
 ...
 * Output: public/projects/live-onboarding-companion-phq0/scene-1/lottie.json
 *
 ...
 * See docs/live-onboarding-companion-phq0-animation.md for the mood retune
...
const OUT_DIR = join(__dirname, '../public/projects/live-onboarding-companion-phq0/scene-1')
...
const FLOAT_POINTS = [
  ...
  { t: 45, v: 1, ease: 'travelBalanced' },
]
const floatEnvelope = (t) => evalTrack(FLOAT_POINTS, 0, CLOCK_FLOAT, ((t % CLOCK_FLOAT) + CLOCK_FLOAT) % CLOCK_FLOAT)
...
// Generalization of evalTrack to a non-periodic, already-boundary-keyed
// points array (e.g. a finished `keyOnBoundaries`-processed track like
// dragPoints) — flat before the first point and after the last, real
// cubic-bezier interpolation between. Used to re-sample an EXISTING motion
// track at an arbitrary/denser cadence for a DERIVED signal (motion-taste.md
// "the world responds": drive a new property off an already-authored track
// instead of hand-curving a duplicate).
function sampleTrackAt(points, t) {
  if (t <= points[0].t) return points[0].v
  if (t >= points[points.length - 1].t) return points[points.length - 1].v
  let seg = points.length - 2
  for (let k = 0; k < points.length - 1; k++) { if (t >= points[k].t && t <= points[k + 1].t) { seg = k; break } }
...
// companion-phq0.svg): plate rect x=14 y=14 w=176 h=35 (bottom edge y=49);
...
// keyOnBoundaries: the gap between the echo's rest tail (well before T) and
// the real run's own anticipate-lead plateau (well after T) already holds
// flat at [0,0] on both sides — but with no keyframe exactly on T, that's
// only an incidental match, not the "key exactly on the loop boundary"
// guarantee motion-taste.md's Living-idles rule requires. Same reasoning
// for `op` below (last real keyframe sits at rest well before 270).
const drinkPoints = keyOnBoundaries([
...
const dragPoints = keyOnBoundaries([
...
const umbrellaRockPoints = keyOnBoundaries([
...
const liquidPoints = keyOnBoundaries([
  { t: 0, v: 0, ease: 'entranceSharp' },
...
// from dragPoints — the SAME track that already drives straw+umbrella's
// rigid drag rotation — dense-resampled via sampleTrackAt and scaled down,
// so the flex is phase-locked to the existing sip-drag beat for free (the
// "the world responds" idiom already used for drink-fill's slosh) rather
// than a hand-authored second curve that could drift out of sync.
const STRAW_BOW_GAIN = 0.5 // px of midpoint lateral bow per "degree" of the drag track
{
  ...
  function strawShapeAt(t) {
    const bow = sampleTrackAt(dragPoints, t) * STRAW_BOW_GAIN

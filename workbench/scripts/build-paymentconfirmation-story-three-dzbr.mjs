 ...
 *
 * THIS RUN (slug dzbr): the three source SVGs are byte-identical to the
 * zezm/h3oo/6v93/4obq field tests (md5-diffed before authoring), and this
 * brief's own language is a word-for-word match of zezm's round-3 (final)
 * brief and of 4obq's own brief — confirmed line by line, not assumed from
 * the slug or file similarity:
 ...
 * applies, and it does. The ribbon gleam uses 4obq's export-safe rebuild — a
 * travelling TRIM WINDOW on the ribbon's own stroke (three low-alpha white
 * strokes sharing one trim window), never a Merge-Paths intersect — per
 * player-contract.md's Export Compatibility section: Merge Paths render
 * correctly in this Skottie preview but paint the raw operand shapes in the
 * HTML export, so the scene ships with zero `ty:'mm'` nodes.
 *
 * Output: public/projects/paymentconfirmation-story-three-dzbr/scene-1/lottie.json
 */
...
const SLUG = 'paymentconfirmation-story-three-dzbr'
...
function handRotationDeg(t) {
  ...
  if (t <= HANDOFF) return easeValue(BEAT1, HANDOFF, SWEEP_DEG, HANDOFF_TOTAL_ROTATION, SPIN_UP, t)
...
const DRAW_START_F = () => ANTIC_END      // same clock as the ribbon's trim-draw
...
function swooshDrawPath() {
  ...
  // Strictly ascending (rounding can collide at these step counts).
  const out = []

#!/usr/bin/env node
/**
 * Generates a seamlessly-looping Lottie JSON for "loop-zenek-our-wn6g.svg"
 ...
 *  scenes; the source SVG's path data is byte-identical — verified all 17
 *  animated paths — so this reuses that proven, documented rig verbatim.
 *  See docs/dataprocessing-loop-animation.md for the full rationale.)
 * Output: public/projects/loop-zenek-our-wn6g/scene-1/lottie.json
 *
...
const OUT_DIR = join(__dirname, '../public/projects/loop-zenek-our-wn6g/scene-1')
...
// ---- pencil (float + tap-down every other cycle) ----
{
  ...
  for (let c = 0; c < FLOAT_CYCLES; c++) {
    ...
    if (isTap) {
      points.push({ t: base + 104, v: [1.5 * A, 2 * A, 0], ease: 'entranceSharp' })
      points.push({ t: base + 112, v: [-1.5 * A, 2 * A, 0], ease: 'entranceSharp' })
      points.push({ t: base + 120, v: [1 * A, 2 * A, 0], ease: 'settleSoft' })
    }
  }
...
// ---- zenek body (eye-white + body outline + body fill, float only) -------
{
  ...
  // Body held completely still — no float motion (static transform).
  const ks = baseTransform()
...
const doc = {
  v: '5.9.0', fr: FPS, ip: 0, op: T, w: W, h: H, nm: 'Loop Zenek — Our (wn6g)',
  ddd: 0,

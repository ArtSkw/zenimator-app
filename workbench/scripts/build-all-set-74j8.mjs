#!/usr/bin/env node
/**
 * Generates an animated Lottie JSON for all-set-74j8.svg — Zenek "all set!"
 * celebration loop. Output: public/projects/all-set-74j8/scene-1/lottie.json
 *
 * Same source illustration as the earlier all-set scene, but a different
 * choreography brief drives a different rig here:
 *  - Real jump: the body-root null lifts UP at each beat's midpoint (airborne
 *    peak, stretched tall, max lean) and returns to its native SVG rest
 *    position at each beat boundary (landing, squashed wide, upright). Push-
 *    off and fall use distinct eases (entrance-sharp up, exit-accelerate
 *    down) for weighted, springy asymmetric motion — see motion-taste.md.
 *  - Blink lands exactly on two of the four airborne peaks (a "wink" at the
 *    top of a jump), not on the landing.
 *  - The two curved strokes above the head are landing-impact puffs (invisible
 *    airborne, pop in at each of the four landings, hold, fade as he springs
 *    back up) — a different job than the old scene's 4th sparkle voice.
 *  - Three sparkle bursts round-robin (200ms stagger, 500ms pulse, 6 cycles/
 *    loop) plus a tiny synthetic companion trail paired with the smallest
 *    sparkle (no source geometry for it — a small hand-authored "+" mark).
 *  - The swoosh ribbon is fully static (no position/scale/rotation/opacity
 *    motion) except one soft light-reflection sweep gliding tail-to-tip,
 *    once per loop. The checkmark tick and spark dot get zero motion,
 *    including the sweep.
 *  - Shadow hatch pulls tight at the airborne peak and spreads wider/softer
 *    at each landing.
...
const OUT_DIR = join(__dirname, '../public/projects/all-set-74j8/scene-1')
...
// Skottie/CanvasKit build (confirmed on the earlier all-set scene, same
// source geometry) — expand to a fill instead of risking it.
...
// ── SVG source data (identical geometry to the earlier all-set scene) ────────
const SVG_PATHS = {
  ...
  // Small hand-authored companion mark — no source geometry for a 4th sparkle
  // voice exists in this SVG (the two curved head strokes are repurposed as
  // landing-impact marks here). A tiny "+" placed just above the smallest
  // sparkle's cluster reads as its trailing companion.
  sparkleTrail:'M101 20L115 20M108 13L108 27',
}
...
const eyeBox = bbox([...P.eye1, ...P.eye2])
const sparkleLeftBox = bbox(P.sparkleLeft)
const sparkleRightBox = bbox(P.sparkleRight)
const sparkleSmallBox = bbox(P.sparkleSmall)
const sparkleTrailBox = bbox(P.sparkleTrail)
const landingMarkBox = bbox(P.landingMark)
const shadowBox = bbox(SHADOW_SEGS)

// ── Beat timing: 4 jumps per loop, L-R-L-R ───────────────────────────────────
const BEAT = FPS * 0.9   // 54 frames per beat
const CENTERS = [0, BEAT, BEAT * 2, BEAT * 3, BEAT * 4]        // landings (ground contact)
const MIDS = [BEAT * 0.5, BEAT * 1.5, BEAT * 2.5, BEAT * 3.5]  // airborne peaks, L R L R
const LEAN = 5.5           // degrees of lean at the airborne peak
const LIFT = 12            // px of real vertical lift at the peak
const STRETCH = { sx: 97, sy: 104 }   // at peaks (tall)
const SQUASH = { sx: 104, sy: 96 }    // at landings (wide/flat, ~4%)

// Push-off (landing → peak) eases fast-out then soft into the hang at the top;
// fall (peak → landing) hangs, then accelerates down — asymmetric per-phase
// easing per motion-taste.md, not a mirrored curve, so the bounce reads
// weighted rather than mechanical.
function bodyRotation() {
  const kfs = []
  const signs = [-1, 1, -1, 1]   // left, right, left, right
  for (let i = 0; i < CENTERS.length; i++) {
    kfs.push({ t: CENTERS[i], s: [0], ease: EASE.entranceSharp })
    if (i < MIDS.length) kfs.push({ t: MIDS[i], s: [LEAN * signs[i]], ease: EASE.exitAccelerate })
  }
  kfs.sort((a, b) => a.t - b.t)
  return anim(kfs, EASE.entranceSharp)
}

function bodyPosition() {
  const kfs = []
  for (let i = 0; i < CENTERS.length; i++) {
    kfs.push({ t: CENTERS[i], s: [PIVOT[0], PIVOT[1], 0], ease: EASE.entranceSharp })
    if (i < MIDS.length) kfs.push({ t: MIDS[i], s: [PIVOT[0], PIVOT[1] - LIFT, 0], ease: EASE.exitAccelerate })
  }
  kfs.sort((a, b) => a.t - b.t)
  return anim(kfs, EASE.entranceSharp)
}
...
function bodyScale() {
  const kfs = []
  for (let i = 0; i < CENTERS.length; i++) {
    kfs.push({ t: CENTERS[i], s: [SQUASH.sx, SQUASH.sy, 100], ease: EASE.settleSoft })
    if (i < MIDS.length) kfs.push({ t: MIDS[i], s: [STRETCH.sx, STRETCH.sy, 100], ease: EASE.settleSoft })
  }
  kfs.sort((a, b) => a.t - b.t)
  return anim(kfs, EASE.settleSoft)
}

// Blink twice per loop (every other beat), landing exactly on the airborne
// peak — MIDS[1] and MIDS[3] — ~120ms thin-line squash and recover.
function blinkScale() {
  const B1 = MIDS[1], B2 = MIDS[3]
  return anim([
    { t: 0,        s: [100, 100, 100], ease: EASE.entranceSharp },
    { t: B1 - 4,   s: [100, 100, 100], ease: EASE.entranceSharp },
    { t: B1,       s: [100,  10, 100], ease: EASE.settleSoft },
    { t: B1 + 4,   s: [100, 100, 100], ease: EASE.entranceSharp },
    { t: B2 - 4,   s: [100, 100, 100], ease: EASE.entranceSharp },
    { t: B2,       s: [100,  10, 100], ease: EASE.settleSoft },
    { t: B2 + 4,   s: [100, 100, 100], ease: EASE.entranceSharp },
...
// A round whose 30f pulse would run past FRAMES is dropped, not clipped —
// letting a generated cycle run past the loop boundary and then forcing an
// earlier "reset to idle" keyframe after it produces a descending t value,
// which silently stops that property from animating at all in this Skottie
// build (confirmed on the earlier all-set scene). Filtering keeps every
// keyframe list strictly ascending.
...
// 36 frames per full round-robin cycle (3 voices × 12f/200ms stagger);
// 216 / 36 = 6 exact round-robin cycles per loop.
const startsLeft = [0, 36, 72, 108, 144, 180]
const startsRight = [12, 48, 84, 120, 156, 192]
const startsSmall = [24, 60, 96, 132, 168, 204]

const pulseLeft = sparklePulse(startsLeft)
const pulseRight = sparklePulse(startsRight)
const pulseSmall = sparklePulse(startsSmall)
// Companion trail rides the smallest sparkle's exact schedule, gentler.
const pulseTrail = sparklePulse(startsSmall, { overshoot: 108, fadeScale: 90 })

// ── Landing-impact puff: invisible airborne, pops in at each of the 4
// landings (0, 54, 108, 162 — the loop wrap covers the 4th↔1st seam), holds
// briefly, fades out with a slight expansion as he springs back up. ────────
function landingPuffAnim() {
  const LANDS = [0, BEAT, BEAT * 2, BEAT * 3]
  const sKfs = [{ t: 0, s: [100, 100, 100], ease: EASE.settleSoft }]
  const oKfs = [{ t: 0, s: [100], ease: EASE.settleSoft }]
  for (let idx = 0; idx < LANDS.length; idx++) {
    const L = LANDS[idx]
    sKfs.push({ t: L + 8,  s: [100, 100, 100], ease: EASE.settleSoft })
    oKfs.push({ t: L + 8,  s: [100], ease: EASE.settleSoft })
    sKfs.push({ t: L + 20, s: [108, 108, 100], ease: EASE.settleSoft })
    oKfs.push({ t: L + 20, s: [0], ease: EASE.settleSoft })

    const nextL = idx + 1 < LANDS.length ? LANDS[idx + 1] : FRAMES
    const preLand = nextL - 3
    sKfs.push({ t: preLand, s: [70, 70, 100], ease: EASE.expressivePop })
    oKfs.push({ t: preLand, s: [0], ease: EASE.settleSoft })
    if (idx + 1 < LANDS.length) {
      sKfs.push({ t: nextL, s: [100, 100, 100], ease: EASE.settleSoft })
      oKfs.push({ t: nextL, s: [100], ease: EASE.settleSoft })
    }
  }
  sKfs.push({ t: FRAMES, s: [100, 100, 100] })
  oKfs.push({ t: FRAMES, s: [100] })
  return { scale: anim(sKfs, EASE.settleSoft), opacity: anim(oKfs, EASE.settleSoft) }
}
const landingPuff = landingPuffAnim()

// ── Shadow: pulls tight at the airborne peak, spreads wider + softer at each
// landing — reads as staying grounded through the jump. ─────────────────────
function shadowScale() {
  const kfs = []
  for (let i = 0; i < CENTERS.length; i++) {
    kfs.push({ t: CENTERS[i], s: [112, 110, 100], ease: EASE.settleSoft })
    if (i < MIDS.length) kfs.push({ t: MIDS[i], s: [86, 84, 100], ease: EASE.settleSoft })
  }
  kfs.sort((a, b) => a.t - b.t)
  return anim(kfs, EASE.settleSoft)
}
function shadowOpacity() {
  const kfs = []
  for (let i = 0; i < CENTERS.length; i++) {
    kfs.push({ t: CENTERS[i], s: [78], ease: EASE.settleSoft })
    if (i < MIDS.length) kfs.push({ t: MIDS[i], s: [100], ease: EASE.settleSoft })
  }
  kfs.sort((a, b) => a.t - b.t)
  return anim(kfs, EASE.settleSoft)
}
...
const outlineLyr = shapeLayer('body-outline', 4, ROOT, [
...
function sparkleLayer(nm, ind, segs, box, pulse, strokeWidth = 1.39197) {
  return shapeLayer(nm, ind, null, [
    group([...segs.map(pathShape), strokeStyle(BLACK, strokeWidth)])
  ], { p: sk([box.cx, box.cy, 0]), a: sk([box.cx, box.cy, 0]), s: pulse.scale, o: pulse.opacity })
}

const sparkleLeftLyr = sparkleLayer('sparkle-left', 8, P.sparkleLeft, sparkleLeftBox, pulseLeft)
const sparkleRightLyr = sparkleLayer('sparkle-right', 9, P.sparkleRight, sparkleRightBox, pulseRight)
const sparkleSmallLyr = sparkleLayer('sparkle-small', 10, P.sparkleSmall, sparkleSmallBox, pulseSmall)
const sparkleTrailLyr = sparkleLayer('sparkle-trail', 11, P.sparkleTrail, sparkleTrailBox, pulseTrail, 1.0)

const landingPuffLyr = shapeLayer('landing-puff', 13, null, [
  group([...P.landingMark.map(pathShape), strokeStyle(BLACK, 1.39197)])
], {
  p: sk([landingMarkBox.cx, landingMarkBox.cy, 0]),
  a: sk([landingMarkBox.cx, landingMarkBox.cy, 0]),
  s: landingPuff.scale,
  o: landingPuff.opacity,
})

const accentFill = fillRule => ({ ty: 'fl', c: { sid: 'accentColor' }, o: sk(100), ...(fillRule === 'evenodd' ? { r: 2 } : {}) })

// Swoosh doodle: ribbon + checkmark tick + spark dot, entirely static — a
// background element behind the character, matching the source geometry
// exactly. No position/scale/rotation/opacity motion on any of it.
...
const doodleLyr = shapeLayer('swoosh-doodle', 15, null, [
...
// Once-per-loop light reflection, ribbon only (tick + dot stay fully static,
// no sweep). A single hard-edged clipped band reads as a straight ruled
// border; a true animated gradient silently renders nothing in this Skottie
// build (confirmed on the earlier all-set/zenimator scenes) — so the soft
// edge is approximated with several identically-colored, low-alpha bands of
// decreasing width, each clipped to the ribbon via its own Merge Paths
// intersect, all sharing one sweep-position animation.
...
const STEP_COUNT = 10, MAX_HALF_WIDTH = 22, MIN_HALF_WIDTH = 2, TARGET_PEAK = 0.32
...
const ribbonSweepLyr = shapeLayer('ribbon-sweep', 14, null, [
...
const shadowLyr = shapeLayer('shadow', 2, null, [
  ...
  p: sk([shadowBox.cx, shadowBox.cy, 0]),
  a: sk([shadowBox.cx, shadowBox.cy, 0]),
  s: shadowScale(),
  o: shadowOpacity(),
})
...
const controls = {
  ...
  layerControls: [
    { target: 'body-root', kind: 'amount', property: 'position',
      label: 'Jump height', description: 'How high Zenek springs at each beat.' },
    { target: 'body-root', kind: 'amount', property: 'rotation',
      label: 'Sway lean', description: 'How far he tilts left/right at the top of each jump.' },
    { target: 'shadow', kind: 'amount', property: 'scale',
      label: 'Shadow pulse', description: 'How much the ground shadow tightens and spreads with the jump.' },
  ],
}

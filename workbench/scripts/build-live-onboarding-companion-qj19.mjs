#!/usr/bin/env node
/**
 * Generates the INTRO+LOOP Lottie for "live-onboarding-companion-qj19.svg" —
 * a live onboarding companion: the mascot hugs its heart-marked pillow, calm
 * reassurance, while a speech bubble above it says its line once, then
 * settles into an endless hug-and-breathe idle. Output:
 * public/projects/live-onboarding-companion-qj19/scene-1/lottie.json
 *
 * Source SVG is BYTE-IDENTICAL to assets/companion-hug.svg AND to
 * assets/live-onboarding-companion-s51p.svg (confirmed via `diff` — only
 * attribute-order/entity-encoding differ). This build reuses
 * scripts/build-live-onboarding-companion-s51p.mjs's rig, entrance, and idle
 * system wholesale — that script already re-derived every constant against
 * the CURRENT skill references in this same session window (Bold text,
 * `bubble.textPos`/`bubble.anchor` slots, `autoFit.max`, house-constant
 * entrance timing, dense Living-idles envelope) and this session independently
 * re-read recipe-companion-bubble.md and motion-taste.md in full and confirmed
 * every one of those constants still matches current doctrine — nothing here
 * is an unaudited port. PORTING IS NOT AUTHORING, so what actually changed
 * from s51p, and why:
 *
 *  - Naming: THIS brief explicitly and repeatedly calls the held object a
 *    "pillow" ("the mascot hugs its heart-marked pillow", "the dark rounded
 *    mass with the white heart is the pillow") — s51p's brief instead named
 *    it a ZEN "charity stone" and the layer names followed that brief. Layer
 *    `nm`s are user-facing (player-contract "Layer Naming": say what the
 *    thing IS) — since the source SVG's own `id="charity stone"` carries no
 *    such meaning in THIS brief, every occurrence is named `pillow` /
 *    `pillow-crease` / `pillow-rig` here, not `charity-stone*`/`stone-rig`.
 *  - Default string: identical between the two briefs ("You're off to a
 *    great start.") — kept as-is, independently confirmed against THIS
 *    task's brief text, not assumed from the port.
 *  - autoFit `max`, entrance timing (T=90/IDLE=216/OP=306), font (Nunito
 *    Bold 15/19), the two plumbing slots, and the dense envelope idle system:
 *    all independently re-checked against this session's fresh read of
 *    recipe-companion-bubble.md and motion-taste.md — same geometry, same
 *    stage, same brief register ("the calmest of the set"), so the same
 *    numbers apply; verified below rather than assumed, see the console
 *    output and the render/pixel-diff checks in this session's transcript.
 *
 * Element map (paint order and geometry confirmed via the byte-diff, not
 * re-guessed):
 ...
 *  - Group 1000007767 -> mascot + the held pillow:
 ...
 *      "charity stone" (+ "_2") -> pillow fill/stroke + its white
 *        creases (pillow-crease)
 *      "Union" -> the heart mark on the pillow; the source stacks TWO white
 *        paths here (path 7: the one true heart, 10 curves; path 8: that
 *        SAME heart's stroke, already expanded to a 26-fragment filled
 *        outline by Figma's export — s51p fitted this to a single clean path
 *        plus a real Lottie stroke at width 5.6469, the SAME width the
 *        source already uses for the pillow's own outline; reused as-is,
 *        same geometry)
 ...
 *    shadow-rig null — confirmed-correct paint order (behind everything) per
 *    companion-hug's raw-SVG-render verification, reused as-is since the
 *    geometry is identical.
 *
 * Rig: "body-rig" (pivot at the body's contact edge with the pillow) parents
 * body/face/eye-left/eye-right — the whole head presses into the pillow each
 * breath. Eyes carry an ADDITIONAL own-center scaleY dip on top (deeper arc
 * at the squeeze). "hug-rig" carries the shared arm-tightening motion and
 * parents arm-ring/paw/arm-crease (pure inherit, no own scale — "never
 * animate the same property twice down a parent chain") PLUS "pillow-rig",
 * which nests inside hug-rig and adds the held pillow's OWN phase-delayed
 * secondary compress, parenting pillow/pillow-crease/heart so they ride the
 * pillow's own deformation ("a held object is part of the body" — see the
 * HUG-RIG + PILLOW-RIG section below). shadow-rig breathes the OPPOSITE sign
 * of the body's envelope, same timing (widens as the body compresses).
 * trail-small / trail-large float gently on their own clocks once the loop
 * begins, so they read as alive satellites instead of freezing into stickers
...
const OUT_DIR = join(__dirname, '../public/projects/live-onboarding-companion-qj19/scene-1')
...
// by diff against assets/live-onboarding-companion-qj19.svg) ─────────────
const SVG_PATHS = {
  ...
  // Source names this element "pillow" (id="pillow" in the
  // SVG) — a ZEN client-benefit Pillow, not a pillow. Renamed from the old
...
// The held pillow: its OWN secondary curve, settling a beat AFTER
...
const PILLOW_B = { rest1: 28, riseEnd: 78, holdEnd: 92, fallEnd: 206 }
function pillowEnvelope(cf) {
  if (cf < PILLOW_B.rest1) return 0
  if (cf < PILLOW_B.riseEnd) return smoothstep((cf - PILLOW_B.rest1) / (PILLOW_B.riseEnd - PILLOW_B.rest1))
  if (cf < PILLOW_B.holdEnd) return 1
  if (cf < PILLOW_B.fallEnd) return 1 - smoothstep((cf - PILLOW_B.holdEnd) / (PILLOW_B.fallEnd - PILLOW_B.holdEnd))
  return 0
}
...
// Body squeeze: volume-preserving-ish (scaleX shrinks toward the pillow's
...
// ============================================================
// HUG-RIG + PILLOW-RIG — "a held object is part of the body" (motion-taste).
// The pillow, its crease, the heart, and every arm part were
...
// pillow-rig, so the whole embrace — arms, paw, and the held pillow — moves
// as one unit. pillow-rig then adds the pillow's OWN secondary compress,
// phase-delayed (pillowEnvelope peaks ~8f after armEnvelope) so the pillow
// settles a beat after the arms, and parents the pillow's own parts (so the
// heart and crease ride the deforming pillow instead of floating over it).
// ============================================================
const armRingSub = parsePath(SVG_PATHS.armRing)
const pillowSub = parsePath(SVG_PATHS.pillow)
const hugBbox = bboxOf([...armRingSub, ...pillowSub])
...
const pillowBbox = bboxOf(pillowSub)
const pillowPivot = [pillowBbox[0], bboxCenter(pillowBbox)[1]] // contact edge (facing the body/arms), mid-height
const pillowScalePts = sampleDense((t) => {
  const e = pillowEnvelope(cyclePos(t))
  return [100 - 3 * e, 100 - 2 * e, 100]
}, 0, OP)
const pillowRigInd = pushLayer({
  nm: 'pillow-rig',
  ty: 3,
  parent: hugRigInd,
  ks: {
    a: { a: 0, k: [pillowPivot[0], pillowPivot[1], 0] },
    p: { a: 0, k: [pillowPivot[0], pillowPivot[1], 0] },
    s: bakedProp(pillowScalePts),
    r: { a: 0, k: 0 },
    o: { a: 0, k: 100 },
  },
})
...
// hug-rig (shared with the paw and the pillow it holds), so arm-ring itself
...
// 5.6469 (a value already used elsewhere in this SVG, for "pillow"'s
...
// reused the pillow's own stroke weight for the heart mark on it.
{
  ...
  pushLayer({ nm: 'heart', shapes, ks: baseTransform(), parent: pillowRigInd })
}
staticShapeLayer('pillow-crease', SVG_PATHS.pillowCrease, [strokeItem('#FFFFFF', 2.21843, 100, 'Stroke', 2, 1)], pillowRigInd)
staticShapeLayer('pillow', SVG_PATHS.pillow, [fillItem('#222222'), strokeItem('#222222', 5.6469, 100, 'Stroke', 0, 0)], pillowRigInd)

// paw: peeks from behind, parented to hug-rig — tightens its grip in step
// with the arm-ring and the pillow, purely by inheriting hug-rig's squeeze.
{
...
const FRONT_TO_BACK = [
  ...
  'arm-crease', 'arm-ring',
  'heart', 'pillow-crease', 'pillow', 'pillow-rig',
...
const doc = {
  v: '5.9.0', fr: FPS, ip: 0, op: OP, w: W, h: H, nm: 'Live Onboarding Companion (qj19)',
  ddd: 0,
  assets: [],
  layers,
  markers: [
...
const controls = {
  ...
  layerControls: [
    ...
    { target: 'hug-rig', kind: 'amount', property: 'scale', label: 'Arm tighten', description: 'How firmly the arms (and the pillow they hold) squeeze at the peak of the hug.' },
    ...
    { target: 'shadow-rig', kind: 'amount', property: 'scale', label: 'Shadow settle', description: 'How much the ground shadow widens as the body settles into the pillow.' },
  ],
}

#!/usr/bin/env node
/**
 * Generates the ENTRY Lottie for "moneytransfer-status-scene-6q7m" — Zenek
 ...
 * Output: public/projects/moneytransfer-status-scene-6q7m/scene-1/lottie.json
 *
 * PRECEDENT CHECK (svg-compatibility.md "diff geometry before rebuilding"):
 * all three source SVGs here are BYTE-IDENTICAL to
 * assets/moneytransfer-status-scene-4b1s{,-2,-3}.svg (confirmed with `diff`,
 * not eyeballed) — same clouds (Group 13272 / Group 48096320), same disc +
 * Zenek + hands (step-2's `Fill 4`/`Zenek` group), same checkmark (Ellipse
 * 2240 ring + Vector_7/Vector_8/Ellipse 2242 ink marks + Stroke 13 tail),
 * same embedded hatch PNG. Geometry, hex colours and gradient stops below are
 * therefore reused as MEASURED FACTS about that shared artwork. The rig
 * topology, every timing/amplitude constant, and the verification pass are
 * re-derived fresh this run against the CURRENT motion-taste.md /
 * recipe-character-rig.md / check-motion.mjs — not ported from
 * build-moneytransfer-status-scene-4b1s.mjs's own rig or numbers, per the
 * brief's explicit "source of GEOMETRY only" instruction.
 *
 * RIG:
 * "bob-rig" (translation null, the whole carried assembly's suspended float
 * + exit travel) parents two ROTATION siblings pivoting at the SAME implied
 * hang point ~90px above the disc's own center:
 *  - "disc-sway": disc outline + hatch + both hand circles (Ellipse 102/103
 *    — "hands gripping the rim" is the brief's own contact weld) parented
 *    rigidly, zero clock of their own.
 *  - "zenek-drag": Zenek's own body (mass + face patch + pupils, wrapped in
 *    a "zenek-breathe" squash null) nests UNDER disc-sway so the shared
 *    pendulum swing is fully inherited (welded, no slide against the disc's
 *    hatch/hands), and carries only a SMALL delta angle of its own, phase-
 *    lagged behind disc-sway — "shares the parent's phase, own softened
 *    curve," never a time-shifted copy of disc-sway's keyframes (motion-
 *    taste, Fluidity — Overlap is drag). Not an occupant-inside-shell case
 *    (gates 15/16): Zenek isn't enclosed/clipped by the disc, he dangles
 *    below it — this is recipe-character-rig's "pendulum-carry" secondary
 *    motion, and the brief itself asks for the lag explicitly.
 * Shadow reads bob-rig's own vertical bob directly for width/opacity
 * (the brief's "on the same driver"), zeroed at rest.
 *
 * The disc/shadow hatch is the SAME raster <pattern> (measured this session
 * from the embedded PNG: 16px pitch / ~3px stroke at the image's native
 * 128px tile) — REVECTORIZED as parallel stroke lines clipped by a track
 * matte of the disc's own circle / the shadow's own ellipse bbox
 * (svg-compatibility.md "Preferred — revectorize the motif"), never
 * flattened to a solid, and precomposed (svg-compatibility.md's multi-beat
 * guidance) so its dozens of line vertices don't register spurious
 * proximity against the checkmark that occupies the same screen region in
 * a LATER, never-simultaneous beat.
 */
...
const SLUG = 'moneytransfer-status-scene-6q7m'
...
// TIMELINE
// ============================================================
const T = 240                                   // FLOAT loop length (4.0s) — highly divisible, see clocks below
const ANTIC_START = T, ANTIC_DUR = 12            // 240-252 opposite lean before the exit
const EXIT_START = ANTIC_START + ANTIC_DUR, EXIT_DUR = 58 // 252-310 accelerate off-frame
const DASH_STRETCH_START = 246, DASH_STRETCH_END = 300
const SHADOW_FADE_START = ANTIC_START, SHADOW_FADE_END = 302
const CLOUD_DECEL_START = 260, CLOUD_DECEL_DUR = 82 // stop by 342
const RING_DRAW = [312, 336]
const RING_START_POP = [312, 322]
const RING_END_POP = [324, 336]
const TAIL_DRAW = [336, 356]
const OP = 368 // ~6.1s @ 60fps — hold 356-368 on the settled checkmark
...
// ============================================================
// Geometry extraction — parse every <path> out of the THREE source SVGs by
// id. A few elements have no `id` under the path-tag parser (the disc's two
// unnamed paths inside `<g id="Fill 4">`, the matrix-transformed hand
// <circle>s, the checkmark's start-dot <circle>) — hand-transcribed below
// from the SVG text, each a short, low-transcription-risk primitive.
// ============================================================
...
// Disc: `<g id="Fill 4">`'s two inner <path>s share this `d` (one pattern-
// filled, one stroked) — no id on either, so transcribed once here.
...
// from the embedded PNG: 16px pitch / ~3px stroke at the image's native
// 128px tile; the tile renders at ~32px on this disc's ~104px-diameter bbox
// (0.307692 objectBoundingBox fraction), giving on-canvas pitch ~4.0px,
// stroke ~0.85px. Colour: the artwork's only explicit ink is #222222
// (matching the pattern's dark sampled tone) — used at 15% opacity to match
// the source's `fill-opacity="0.15"`.
// ============================================================
...
// matted, but real geometry that check-motion.mjs still measures against
// nearby unrelated elements).
...
// Shadow: clip to the bbox with zero margin (an ellipse — exact-bbox
...
// ============================================================
// PIVOT — the implied hang point, ~90px above the disc's own center.
// Small-angle sway about this point gives a readable few-px lateral swing
// for a "few degree" tilt (motion-taste "Amplitude that reads at arm's
// length").
// ============================================================
...
const SWAY_PERIOD = 240   // 1 cycle per FLOAT loop — slow pendulum, "nothing bouncy"
const SWAY_AMP = 3.4      // deg
const BOB_PERIOD = 48     // 5 cycles per FLOAT loop (240/48 = 5)
const BOB_AMP_X = 1.5, BOB_AMP_Y = 3.8 // px
const BREATHE_PERIOD = 30 // 8 cycles per FLOAT loop (240/30 = 8) — cycle ratio 1:5:8, not 1:2:4
const BREATHE_AMP = 2.8   // % non-uniform scale swell (area-conserving pair)
...
// Bounded so its lever-arm displacement at Zenek's own farthest feature (his
// pupil — ~0.2px from the disc's rim at rest in the artwork, the eye is
// drawn right at the canopy's edge) stays under check-motion.mjs's 0.75px
// weld tolerance: leverArm(~22.5px) * sin(peakAngle) must clear that margin.
const ZENEK_DELTA_LAG_DEG = 34 // phase lag behind disc-sway's own phase
const ZENEK_DELTA_AMP = 1.4    // deg — small secondary wobble on top of the inherited sway

function swayAngle(t) {
  const antic = -0.55 * SWAY_AMP * bump(t, ANTIC_START, ANTIC_DUR) // small anticipation lean opposite
  const exitTilt = ramp(t, EXIT_START, EXIT_DUR, 19, easeInQuad)    // tilts into the direction of travel
...
function exitX(t) {
  const antic = -6.5 * bump(t, ANTIC_START, ANTIC_DUR)
  return (t <= ANTIC_START ? 0 : antic) + ramp(t, EXIT_START, EXIT_DUR, 555, easeInQuad)
}
function exitY(t) {
  const antic = 4 * bump(t, ANTIC_START, ANTIC_DUR)
  return (t <= ANTIC_START ? 0 : antic) + ramp(t, EXIT_START, EXIT_DUR, -145, smoothstep)
}
...
// centroid for the small extra delta wobble.
const zenekBodyBbox = bboxOf([...subs('Fill 1')])
const zenekPivot = bboxCenter(zenekBodyBbox)
const zenekDragPts = sampleDense((t) => zenekDeltaAngle(t), 0, OP)
const zenekDragInd = pushNull({
  nm: 'zenek-drag', parent: discSwayInd,
  ks: { a: { a: 0, k: [zenekPivot[0], zenekPivot[1], 0] }, p: { a: 0, k: [zenekPivot[0], zenekPivot[1], 0] }, s: { a: 0, k: [100, 100, 100] }, r: bakedProp(zenekDragPts), o: { a: 0, k: 100 } },
})
...
// ============================================================
// CHECKMARK (frontmost — pushed first). Ring draws first with its own
// pen-down/pen-up ink marks, then the tail draws.
// ============================================================
{
  // Source path is authored right-to-left (starts at the upper-right tip,
  // ends at the lower-left) — reversed so the trim-from-start reveal draws
  // pen-down to pen-up, left to right (motion-taste "Ink follows the pen").
...
{
  // The ellipse shape sits at the group's own local origin (0,0) — origin-
...
// ============================================================
// SHADOW — reads bob-rig's own vertical bob directly: wider + lighter as
// the assembly rises (bobY negative = risen on screen), zeroed at rest.
// Fades out during the exit. Revectorized hatch, matted to the shadow's own
// flat-ellipse silhouette.
// ============================================================
const SHADOW_SCALE_AMP = 9.5, SHADOW_OP_AMP = 15, SHADOW_BASE_OP = 40
...
// fade carried on both halves of the matte pair so the clipped edge doesn't linger visible
...
// zenek-breathe's own scale swell; pupils carry a shared blink on top.
// ============================================================
// Blink: down / hold-closed / up, centered on the sway's own apex (T/4, a
// momentary rest beat for a sine pendulum) — whole accent 24f = 0.4s.
const BLINK_DOWN = Math.round(T / 4) - 12, BLINK_HOLD = BLINK_DOWN + 8, BLINK_UP = BLINK_HOLD + 8, BLINK_END = BLINK_UP + 8
...
// ============================================================
// CLOUDS — the SAME set kept alive across the whole timeline. Constant-
// velocity wrap during FLOAT completes a WHOLE number of laps in exactly T
// frames (closes the loop by construction); continues into SUCCESS at the
// same exit velocity, then eases to a full stop. Near cloud completes 2
// laps in T, far cloud 1 — near is therefore mechanically faster, free
// parallax with no separate speed constant.
// ============================================================
...
function continueAndStop(loopPts, exitVelocity, exitSign) {
  // Continue at FLOAT's own exit velocity from T (constant, so velocity is
  // continuous crossing the T seam), then decelerate smoothly
  // (velocity(u) = v*(1-u)^2, numerically integrated) to a full stop.
  const preStopPts = []
...
function cloudLayer(nm, ids, wrap) {
  ...
  const stretchPts = sampleDense((t) => {
    ...
    return [100 + 155 * Math.pow(b, 1.4), 100, 100]
  }, 0, OP)
  const shapes = [
...
// Native bboxes: near x[9.381,153.382], far x[232.734,362.001] — 375-wide
// canvas, EXIT/ENTER offsets sized so the wrap happens fully offscreen with
// clearance on both sides of the jump.

 ...
 *
 * THIS RUN (slug wyga): the three source SVGs are byte-identical (md5-diffed
 * before authoring) to the zezm/h3oo/6v93/4obq/dzbr field tests, and this
 * brief's own language is a word-for-word match of zezm's round-3 (final)
 * brief and of 4obq's/dzbr's own briefs — confirmed line by line against the
 * prompt text, not assumed from the slug or file similarity:
 ...
 * Base script: 4obq (not dzbr directly) — 4obq is the current, export-safe,
 * ONE-PEN-sequential-draw descendant in this lineage (dzbr predates that
 * refinement). The green line's two halves (ribbon then swoosh) chain pen
 * order end-to-start with length-proportional durations — the fix promoted
 * into motion-taste.md's connected-line-is-one-gesture bullet this session —
 * not the earlier both-halves-radiate-from-the-join build dzbr/h3oo/6v93 ship.
 ...
 * Compatibility section, motion-taste's Aliveness Contract gates 1-19 AND its
 * updated connected-line bullet, chapterization's Grounded Handoffs checklist)
 ...
 * applies, and it does.
 *
 * Output: public/projects/paymentconfirmation-story-three-wyga/scene-1/lottie.json
 */
...
const SLUG = 'paymentconfirmation-story-three-wyga'
...
//
// This pass overlaps them instead. His anticipation (coil) starts at the
// BEGINNING of the swoosh's own last third — offscreen, so there is still no
// visual collision with the stroke — and his rise takes over the INSTANT the
// pen runs out: `ANTIC_END` is pinned to `DRAW_END` exactly, so the last
// frame of the drawn line and the first frame of the rise are the same
// frame, with no gap between them. The spark pops right at that same
// handoff (pen-lift, its original convention — see sparkEntrance below), so
// something is already resolving on stage the instant the stroke finishes;
// the mascot's rise/land keeps its own unchanged pace (50f rise, 22f land)
// and lands after the spark has already settled, closing the phrase.
...
const ANTIC_START = DRAW_END - LAST_THIRD   // anticipation begins here, still offscreen
const ANTIC_END = DRAW_END                  // rise takes over the instant the pen runs out
const RISE_END = ANTIC_END + 50             // overshoot peak, unchanged pace
const LAND_END = RISE_END + 22              // settle, unchanged pace — lands after the spark

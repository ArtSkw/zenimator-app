 ...
 *
 * THIS RUN (slug 4pik): the three source SVGs are byte-identical (md5-diffed
 * before authoring) to the zezm/h3oo/6v93/4obq/dzbr/wyga field tests, and
 * this brief's own language is a word-for-word match of zezm's round-3
 * (final) brief and of 4obq's/dzbr's/wyga's own briefs — confirmed line by
 * line against the prompt text, not assumed from the slug or file
 * similarity:
 ...
 *     "take the length beat 3 needs", loop ~216f) match zezm's shipped
 *     BEAT1/2 (72/104) exactly; beat 3's own length is derived, not fixed
 *     (see wyga's Edit 1/Edit 2 history below).
 *   - the entrance/gesture-overlap wording ("Overlap them, never queue
 *     them... no frame where the stroke has finished and the next action has
 *     not begun") matches wyga's brief verbatim — the exact language that
 *     produced wyga's Edit 2 fix (anticipation starting in the swoosh's own
 *     last third, rise taking over the instant the pen runs out).
 * Base script: wyga — the most current, field-tested, TWICE-edited
 * descendant in this lineage (4obq established the ONE-PEN sequential-draw
 * fix; wyga's Edit 1 fixed the mascot covering the still-drawing swoosh,
 * and Edit 2 fixed the pen-finish-to-jump handoff reading as two queued
 * events instead of one overlapping phrase — both defects this brief's own
 * wording explicitly guards against, quoted above). dzbr/h3oo/6v93 predate
 * these fixes and are not the base.
 ...
 * (player-contract.md's opacity-does-not-cascade/precomp note + Export
 * Compatibility section, motion-taste's Aliveness Contract gates 1-19
 * unchanged since wyga's run, chapterization's Grounded Handoffs checklist
 * including the "arrival must not eclipse a gesture" / "overlap, never
 * queue" clauses wyga's Edit 2 already implements) — not carried on the
 ...
 * it does.
 *
 * Output: public/projects/paymentconfirmation-story-three-4pik/scene-1/lottie.json
 */
...
const SLUG = 'paymentconfirmation-story-three-4pik'
...
//
// Round 1 let the mascot's rise start as early as ANTIC_END = POP_START+10,
// well before the swoosh was even half drawn, so his rising body climbed
// straight over it. Round 2 fixed that by withholding his rise until 10f
// after the swoosh's own last stretch — but that overcorrected: with nothing
// on stage between the completed line (DRAW_END) and his first visible
// entrance ~14f later, and the spark itself pushed all the way out to sync
// with his landing, the pen finishing read as one event and the jump as a
// second, unrelated one, with dead air between them.
//
// This pass overlaps them instead. His anticipation (coil) starts a fixed
// lead ahead of the pen-lift — offscreen, so there is still no visual
// collision with the stroke — and his rise takes over `VISIBILITY_LEAD`
// frames before the pen runs out.
//
// EDIT (this session, designer note: "he reads as arriving after the green
// line — his anticipation happens below the frame where nobody sees it").
// Anchored to the pen-lift (`ANTIC_END = DRAW_END`), the coil-to-launch
// segment used to ride `exitAccelerate` (a near-zero start slope) across the
// full offscreen-to-overshoot travel — so although the RISE keyframe landed
// exactly on the pen-lift frame, the easing kept him invisible for ~34 more
// frames after it (measured by dense-sampling the shipped position track
// against the frame edge: crosses visible at ANTIC_END+34, not at
// ANTIC_END). The fix anchors to VISIBILITY instead of to the pen-lift:
//   1. `ANTIC_END` moves `VISIBILITY_LEAD` (20f) earlier, so the whole
//      entrance — coil, rise, landing — shifts ~20f earlier, per the
//      designer's "shift by about twenty frames."
//   2. The coil-to-launch segment's ease swaps `exitAccelerate` ->
//      `settleSoft` (a near-vertical start slope) on both the position and
//      rotation tracks — mascotScaleKfs already used `settleSoft` here, so
//      this brings position/rotation into line with what scale was already
//      doing. A real jump's takeoff is fast and decelerates into the peak
//      (gravity), so this reads as more physically correct, not just a
//      visibility hack. Measured against the same frame-edge test: crosses
//      visible only 2f after `ANTIC_END` now, so with the 20f-earlier
//      anchor he clears the bottom edge at ANTIC_END+2 = DRAW_END-18 — a
//      couple of frames before the swoosh's own last third begins
//      (DRAW_END-LAST_THIRD = DRAW_END-17), and is visibly rising well
//      before the swoosh is two-thirds drawn.
// The spark still pops at the pen-lift (its own convention, unchanged — see
// sparkEntrance below); the mascot's rise/land keeps its own unchanged
// amplitude (50f rise, 22f land) and now lands well clear of the spark's own
// settle, closing the phrase with the same sequencing as before, just moved
// earlier.
...
const VISIBILITY_LEAD = 20                  // designer: "shift his whole entrance earlier by about twenty frames"
...
function mascotPosition() {
  ...
  const kfs = [
    ...
    // ANTIC_END, not ~34f later once the easing finally picks up (see the
    // TIMELINE comment above). A fast liftoff decelerating into the
    // overshoot also reads as the more physical jump.
...
// slowdown at the handover reads as the hand rounding a corner. Timing
// (`DRAW_START`/`DRAW_END`/`RIBBON_DRAW_START`/`RIBBON_DRAW_END`) is computed
// in the TIMELINE section above, ahead of the mascot's own entrance — the
// green line draws on an empty stage first, and his launch is timed off it.
...
function sparkEntrance() {
  ...
  // DRAW_END), so something is already resolving on stage the moment the
  // line completes. He lands after the spark has settled, not with it.
  const c = DRAW_END - 2
  const s = anim([

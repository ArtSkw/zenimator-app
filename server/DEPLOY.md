...
1. **A Claude Code OAuth token** (subscription auth — no metered API key):
   ```
   claude setup-token
   ```
...
CLAUDE_CODE_OAUTH_TOKEN=<the claude setup-token value>
...
- Generation: your Claude **subscription** via the OAuth token — no per-call API
  billing. Set the app's Settings → Model to `claude-sonnet-5` (the default) to

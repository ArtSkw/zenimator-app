...
const LIVING_MOTION_CONTRACT =
  ...
  `  - THE BRIEF OUTRANKS EVERY GATE. Source fidelity is the DEFAULT — what a scene should do when the brief is SILENT — not a law. The checks exist to catch what you did NOT intend. When the brief says something flies away, fades out, walks off, scatters, or that the frame empties, THAT is the correct final state and a gate objecting to it is the thing that is wrong: declare it in controls.json quoting the brief's own sentence as the reason, and never reshape the motion to make a check pass. A scene that satisfies every gate while contradicting the brief has failed at the only thing that matters.\n` +

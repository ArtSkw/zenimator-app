...
export function ParamSelect({ label, value, options, originValue, onChange }: Props) {
  ...
  // Show the selected option's LABEL in the trigger (base-ui's SelectValue
  // otherwise renders the raw value — a bare number for our numeric options).
      ...
      <div className="group space-y-1.5">
        ...
        <Select value={value} onValueChange={(v) => { if (v != null) onChange(v) }}>
          <SelectTrigger className="h-7 w-full text-[11px] bg-background">
            <span data-slot="select-value" className="line-clamp-1 flex-1 text-left">{currentLabel}</span>
          </SelectTrigger>
          <SelectContent>
            {options.map((o) => (

...
import { elementWorldMatrix, transformRect } from './transform';

const VISUAL_TAGS = new Set([
  'path', 'rect', 'circle', 'ellipse', 'polygon', 'polyline', 'line',
  'use', 'text', 'image',
]);
const SKIP_CONTAINERS = new Set(['defs', 'clippath', 'mask', 'symbol', 'filter', 'marker', 'pattern']);
...
function walkAndIndex(root: SVGSVGElement): StructuralElement[] {
  ...
  const walk = (el: Element, parentId: string | null) => {
    ...
    if (shouldRecord) {
      ...
      // round-trip through the LLM, CSS selectors, or both. The original name
      // is kept as data-zen-src-id: it's the illustrator's own label for the
      // element (e.g. a group's "cloud left"), which downstream digests prefer
      // over an opaque synthetic id.
      if (!el.id || !CLEAN_ID.test(el.id)) {
        if (el.id) el.setAttribute('data-zen-src-id', el.id);
        el.id = nextSyntheticId();
      }
      elementId = el.id;

      let bounds: Rect = { x: 0, y: 0, width: 0, height: 0 };
      if (isGraphicsElement(el)) {
        // getBBox() is LOCAL (pre-transform); bake the element's transform chain
        // so bounds land in root space — matches the now-transformed geometry and
        // fixes selection boxes for elements positioned by a `transform`.
        bounds = transformRect(elementWorldMatrix(el), getBounds(el));
      }

 ...
 * (base64 `data:` payloads inside `<image>` elements).
 *
 * LEGACY-ENGINE ONLY (generateGroundedLottie). The v3 skeleton pipeline
 * (buildSkeleton) accepts embedded rasters: each becomes a raster image leaf
 * that can move, fade, and matte-reveal — only trim-path draw-on needs true
 * vector geometry.
 */
